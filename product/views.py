...
# Подключение моделей
from .models import Organization, Invoice, Category, Catalog, ViewCatalog, Basket, Sale, ViewSale, Delivery, Question, News
# Подключение форм
from .forms import OrganizationForm, InvoiceForm, CategoryForm, CatalogForm, DeliveryForm, ReviewForm, QuestionCreateForm, QuestionEditForm, NewsForm, SignUpForm
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if 'searchBtn' in request.POST:
                ...
                return render(request, "catalog/list.html", {"catalog": catalog, "category": category, "mess": message, "basket_count": basket_count, "basket_total": basket_total, })         

        else:
            return render(request, "catalog/list.html", {"catalog": catalog, "category": category, "basket_count": basket_count, "basket_total": basket_total,})            
    except Exception as exception:
        print(exception)
        return HttpResponse(exception)
...
def catalog_edit(request, id, invoice_id):
    try:
        ...
        if request.method == "POST":
            ...
            # Загрузка начальных данных
            catalogform = CatalogForm(initial={'category': catalog.category, 'catalog_title': catalog.catalog_title, 'details': catalog.details, 'price': catalog.price, 'quantity': catalog.quantity, 'unit': catalog.unit, 'photo': catalog.photo })
            #print('->',catalog.photo )
            return render(request, "catalog/edit.html", {"form": catalogform, "invoice_id": invoice_id})
